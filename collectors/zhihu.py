    ...
    def _search(self, keyword: str) -> int:
        try:
            result = subprocess.run(
                ["opencli", "zhihu", "search", "--keyword", keyword, "--limit", str(self.limit)],
                capture_output=True, text=True, timeout=120,
            )
            return self._process_result(result, f"搜索:{keyword}")
        except FileNotFoundError:
            print("[WARN] Zhihu: opencli zhihu command not available")
            return 0
        except Exception as e:
            print(f"[ERROR] Zhihu search '{keyword}': {e}")
            return 0

    def _process_result(self, result: subprocess.CompletedProcess, detail: str) -> int:
        if result.returncode != 0:
            print(f"[WARN] Zhihu {detail}: {result.stderr[:200]}")
            return 0
