...
```json
[
  {{"index": 1, "category": "分类名", "score": 7, "summary": "一句话摘要"}},
  {{"index": 2, "category": "分类名", "score": 5, "summary": "一句话摘要"}}
]
```"""

    response = call_llm(prompt, system="你是一个信息分析助手，擅长快速评估信息价值。")
    ...
    try:
        ...
        return [
            {**item, "category": "其他", "score": 3.0, "summary": item.get("title", "")}
            for item in items
        ]
...
def generate_report(categories_items: dict[str, list[dict]], date_str: str) -> str:
    ...
    for category, items in categories_items.items():
        ...
        for item in sorted(items, key=lambda x: x.get("score", 0), reverse=True):
            ...
            lines.append(f"- {score_emoji} **[{score}/10]** [{title}]({url})")
            if source_detail:
